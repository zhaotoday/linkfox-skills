...
| 项 | 值 |
|----|-----|
| **脚本** | `scripts/global_seller_cancel_order_cancel_appeal_apply.py` |
...
> `reason.proofUrlList` 中的图片须先经 **`temu.order.query.signature`** 转为 URL 后再传入（该接口待接入时可先用 `temu_global_proxy.py` 试调）。
...
```bash
python scripts/global_seller_cancel_order_cancel_appeal_apply.py '{
  "accessToken": "TOKEN",
  "tokenPurpose": "order-shipping",
  "request": {
    "cancelType": 3,
