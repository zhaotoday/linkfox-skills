...
| 项 | 值 |
|----|-----|
| **脚本** | `scripts/global_seller_cancel_order_cancel_outofstock_result_get.py` |
...
> `parentOrderSn`、`orderSnList` 通常来自 `linkfox-temu-order-global` 订单列表/详情接口，或与 [temu-order-cancel-outofstock-apply.md](./temu-order-cancel-outofstock-apply.md) 提交时使用的参数一致。
...
```bash
python scripts/global_seller_cancel_order_cancel_outofstock_result_get.py '{
  "accessToken": "TOKEN",
  "tokenPurpose": "order-shipping",
  "request": {
    "parentOrderSn": "PO-123456789",
