...
| 项 | 值 |
|----|-----|
| **脚本** | `scripts/us_cancel_aftersales_cancel_agree.py` |
...
```bash
python scripts/us_cancel_aftersales_cancel_agree.py '{
  "accessToken": "TOKEN",
  "tokenPurpose": "order-shipping",
  "request": {
    "parentAfterSalesSn": "PAS-123456789",
...
典型流程：`bg.aftersales.cancel.list.get`（`afterSalesStatusGroup=8`）取得 `parentAfterSalesSn` + `parentOrderSn` → 本接口**同意取消** → 可再查列表确认状态变为已同意（`9`）或用 `linkfox-temu-order-us` 刷新订单状态。
