...
| 项 | 值 |
|----|-----|
| **脚本** | `scripts/us_cancel_aftersales_cancel_list_get.py` |
...
可与 `linkfox-temu-order-us` 的 `bg.order.list.v2.get` / `bg.order.detail.v2.get` 配合：先查订单 `pending_buyer_cancellation` 标签，再按 `parentOrderSnList` 拉取消售后明细。
