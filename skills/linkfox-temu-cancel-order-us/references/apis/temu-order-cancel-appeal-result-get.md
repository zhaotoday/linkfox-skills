...
| 项 | 值 |
|----|-----|
| **脚本** | `scripts/us_seller_cancel_order_cancel_appeal_result_get.py` |
...
```bash
python scripts/us_seller_cancel_order_cancel_appeal_result_get.py '{
  "accessToken": "TOKEN",
  "tokenPurpose": "order-shipping",
  "request": {
    "applySnList": ["APPLY-SN-123456789"]
...
典型流程：`temu.order.cancel.appeal.apply` 提交申请取得 `applySn` → 本接口轮询 `status`（`1` 审核中 → `2` 通过 / `3` 拒绝 / `4` 失败 / `5` 已取消）→ 可用 `linkfox-temu-order-us` 刷新订单状态。
