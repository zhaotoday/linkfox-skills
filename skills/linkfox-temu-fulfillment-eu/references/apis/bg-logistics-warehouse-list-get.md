...
| 项 | 值 |
|----|-----|
| **脚本** | `scripts/eu_buy_shipping_logistics_warehouse_list_get.py` |
...
> 下游 **`linkfox-temu-fulfillment-eu`** 的 `bg.logistics.shipment.v2.confirm` 等接口中的 **`selfShippingWarehouseId`** 可来自本接口返回的 **`warehouseId`**。
...
4. linkfox-temu-fulfillment-eu → selfShippingWarehouseId 取自 warehouseId
```
...
# 仅返回支持购标的仓库
python scripts/eu_buy_shipping_logistics_warehouse_list_get.py '{
  "accessToken": "TOKEN",
  "tokenPurpose": "order-shipping",
  "request": {
    "returnEnableBuyShippingLabelOnly": true
