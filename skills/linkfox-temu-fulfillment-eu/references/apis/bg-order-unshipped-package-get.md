...
| 项 | 值 |
|----|-----|
| **脚本** | `scripts/eu_buy_shipping_order_unshipped_package_get.py` |
...
> **关联能力：** 购标后包裹号来自 [**`bg.logistics.shipment.create`**](./bg-logistics-shipment-create.md)；订单号来自 **`linkfox-temu-order-eu`**；确认发货见 [**`bg.logistics.shipped.package.confirm`**](./bg-logistics-shipped-package-confirm.md)。
...
```text
1. linkfox-temu-order-eu                         → 待发货父/子订单号
...
python scripts/eu_buy_shipping_order_unshipped_package_get.py '{
  "accessToken": "TOKEN",
  "tokenPurpose": "order-shipping",
  "request": {
    "pageNumber": 1,
