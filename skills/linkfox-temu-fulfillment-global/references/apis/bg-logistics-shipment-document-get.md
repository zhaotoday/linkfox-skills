...
| 项 | 值 |
|----|-----|
| **脚本** | `scripts/global_buy_shipping_logistics_shipment_document_get.py` |
...
> 返回的 **`url`** 若为加签链接，可用本 skill 的 **`temu_global_file_download.py`**（`POST /temu/fileDownload`）下载 PDF/PNG 文件。
...
| url | STRING | Document download URL（面单/运单文档下载地址；用于打印或经 **`temu_global_file_download.py`** 拉取文件） |
...
4. temu_global_file_download.py（可选）         → 下载加签 PDF/PNG
5. 打印面单 / 出库
```
...
python scripts/global_buy_shipping_logistics_shipment_document_get.py '{
  "accessToken": "TOKEN",
  "tokenPurpose": "order-shipping",
  "request": {
    "documentType": "SHIPPING_LABEL_PDF",
...
```bash
# 下载面单（若 url 需网关加签下载）
python scripts/temu_global_file_download.py '{
  "accessToken": "TOKEN",
  "tokenPurpose": "order-shipping",
