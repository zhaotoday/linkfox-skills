...
| 项 | 值 |
|----|-----|
| **脚本** | `scripts/global_buy_shipping_logistics_shippingservices_get.py` |
...
> **前置依赖：** `warehouseId` 来自 **`bg.logistics.warehouse.list.get`**；`parentOrderSn` / `orderSn` 来自 **`linkfox-temu-order-global`**。购标下一步通常使用返回的 **`channelId`**、**`shipLogisticsType`** 等调用 **`bg.logistics.shipment.create`**。
...
2. linkfox-temu-order-global               → parentOrderSn、orderSn、数量
...
python scripts/global_buy_shipping_logistics_shippingservices_get.py '{
  "accessToken": "TOKEN",
  "tokenPurpose": "order-shipping",
  "request": {
    "warehouseId": "WH-001",
