...
| 项 | 值 |
|----|-----|
| **脚本** | `scripts/global_tracking_track_trackinginfo_get.py` |
...
> **关联能力：** **`packageSn`** 来自 **`linkfox-temu-fulfillment-global`**（如 **`bg.logistics.shipment.create`** / **`bg.logistics.shipment.result.get`**）；订单上下文见 **`linkfox-temu-order-global`**。  
...
```text
1. linkfox-temu-fulfillment-global  → 购标获得 packageSn / trackingNumber
2. temu.track.trackinginfo.get（本接口）     → 按 packageSn 查询物流轨迹详情
3. linkfox-temu-order-global                     → 结合订单状态做履约跟进
```
...
python scripts/global_tracking_track_trackinginfo_get.py '{
  "accessToken": "TOKEN",
  "tokenPurpose": "order-shipping",
  "request": {
    "packageSn": "PKG-001",
