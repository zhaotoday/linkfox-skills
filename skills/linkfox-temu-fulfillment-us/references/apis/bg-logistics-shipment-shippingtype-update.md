...
| 项 | 值 |
|----|-----|
| **脚本** | `scripts/us_self_fulfilled_logistics_shipment_shippingtype_update.py` |
...
> - `selfShippingWarehouseId` 来自 **`linkfox-temu-fulfillment-us`** 的 **`bg.logistics.warehouse.list.get`**
...
| selfShippingWarehouseId | STRING | **是** | The shipment warehouse ID（发货仓库 ID），可从 **`linkfox-temu-fulfillment-us`** 的 **`bg.logistics.warehouse.list.get`** 获取 |
