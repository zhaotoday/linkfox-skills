...
| 项 | 值 |
|----|-----|
| **脚本** | `scripts/us_self_fulfilled_logistics_shipment_sub_confirm.py` |
...
> - `mainPackageSn` 来自 **`bg.logistics.shipment.get`**（待接入时可先用 `temu_us_proxy.py`）  
> - `selfShippingWarehouseId` 来自 **`linkfox-temu-fulfillment-us`** 的 **`bg.logistics.warehouse.list.get`**（`warehouseId`）  
...
| selfShippingWarehouseId | STRING | **是** | The shipment warehouse ID（发货仓库 ID），可从 **`linkfox-temu-fulfillment-us`** 的 **`bg.logistics.warehouse.list.get`** 返回的 **`warehouseId`** 获取 |
...
```bash
python scripts/us_self_fulfilled_logistics_shipment_sub_confirm.py '{
  "accessToken": "TOKEN",
  "tokenPurpose": "order-shipping",
  "request": {
    "mainPackageSn": "PKG-MAIN-123456789",
